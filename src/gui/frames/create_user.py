...
class CreateUserFrame(customtkinter.CTkFrame):
    ...
    def _build(self):
        ...
        # Role
        customtkinter.CTkLabel(card, text="Role", anchor="w").grid(
        ...
        self._pwd.grid(row=3, column=1, padx=(0, 36), pady=8)
        ...
        self._confirm.grid(row=4, column=1, padx=(0, 36), pady=8)
        self._confirm.bind("<Return>", lambda _e: self._submit())

        # Status (error / success)
        self._status = customtkinter.CTkLabel(card, text="", text_color="#FF5555")
        self._status.grid(row=5, column=0, columnspan=2, padx=36, pady=(4, 0))

        # Buttons
        btn_row = customtkinter.CTkFrame(card, fg_color="transparent")
        btn_row.grid(row=6, column=0, columnspan=2, padx=36, pady=(12, 30))
    ...
    def _submit(self) -> None:
        ...
        self._manifest.create_user(username, password, role)
