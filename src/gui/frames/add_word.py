...
class AddWordFrame(customtkinter.CTkFrame):
    ...
    def _build(self):
        ...
        customtkinter.CTkLabel(port_panel, text="Arduino", anchor="w").grid(
            row=0, column=0, padx=(14, 8), pady=10, sticky="w"
        )
        self._port_var = customtkinter.StringVar(value="")
        self._port_menu = customtkinter.CTkOptionMenu(
            port_panel, values=[""], variable=self._port_var,
            width=180, command=self._on_port_change,
        )
        self._port_menu.grid(row=0, column=1, padx=(0, 6), pady=10, sticky="ew")

        customtkinter.CTkButton(
            port_panel, text="↻", width=40, command=self._refresh_ports
        ).grid(row=0, column=2, padx=(0, 14), pady=10)

        # ── Record button + progress ─────────────────────────────────────────
        self._record_btn = customtkinter.CTkButton(
            card, text=f"● Record {_RECORD_SECONDS} sec", width=240, height=40,
            fg_color="#B33A3A", hover_color="#8C2A2A",
            command=self._record,
        )
        self._record_btn.grid(row=4, column=0, padx=20, pady=(8, 4))

        # Verify panel — shown only while a pending recording is held in memory.
        ...
        self._verify_lbl = customtkinter.CTkLabel(
            self._verify_panel, text="Verify recording before saving",
            font=customtkinter.CTkFont(size=12, weight="bold"),
        )
        self._verify_lbl.grid(row=0, column=0, columnspan=3, padx=8, pady=(4, 6))

        self._play_pending_btn = customtkinter.CTkButton(
            self._verify_panel, text="▶ Play", width=78,
            command=self._play_pending,
        )
        self._play_pending_btn.grid(row=1, column=0, padx=4, pady=(0, 4))

        self._redo_btn = customtkinter.CTkButton(
            self._verify_panel, text="↻ Redo", width=78,
            fg_color="gray30", hover_color="gray20",
            command=self._redo_pending,
        )
        self._redo_btn.grid(row=1, column=1, padx=4, pady=(0, 4))

        self._save_btn = customtkinter.CTkButton(
            self._verify_panel, text="✓ Save", width=78,
            fg_color="#2A8C2A", hover_color="#226B22",
            command=self._save_pending,
        )
        self._save_btn.grid(row=1, column=2, padx=4, pady=(0, 4))

        self._progress_bar = customtkinter.CTkProgressBar(card, width=240)
        self._progress_bar.set(0)
        self._progress_bar.grid(row=5, column=0, padx=20, pady=(4, 4))

        self._status_lbl = customtkinter.CTkLabel(
            card, text="", text_color="gray",
            font=customtkinter.CTkFont(size=11),
        )
        self._status_lbl.grid(row=6, column=0, padx=20, pady=(0, 8))

        customtkinter.CTkButton(
            card, text="Back", width=120,
            fg_color="gray30", hover_color="gray20",
            command=lambda: self.handler.show("main_menu"),
        ).grid(row=7, column=0, padx=20, pady=(4, 20))
    ...
    def _refresh_word(self) -> None:
        ...
        self._word_meta_lbl.configure(
            text=f"Word {idx + 1} of {total}  ·  {recorded} sample(s)"
        )
    ...
    def _do_record(self, word: str, port: str, baud: int) -> None:
        ...
        try:
            with ArduinoSerial(port, baud=baud) as ard:
                # Signal only after connection succeeds (better than at thread start)
    ...
    def _on_record_done(self, word, pcm, sample_rate, err) -> None:
        ...
        self._set_status(
            f"Captured {_RECORD_SECONDS}s for '{word}'. "
            "Play to verify, Redo to retry, Save to keep.",
            error=False,
        )
    ...
    def _show_verify_panel(self) -> None:
        ...
        self._verify_panel.grid(row=4, column=0, padx=20, pady=(8, 4), sticky="ew")
    ...
    def _save_pending(self) -> None:
        if (
            self._pending_pcm is None
            or self._pending_sample_rate is None
            or not self._pending_word
            or not self._recordings
            or not self._progress
        ):
            return
        word = self._pending_word
        try:
            path = self._recordings.save_sample(
                word, self._pending_pcm, self._pending_sample_rate
            )
        except OSError as e:
    ...
    def _read_cfg(self, key: str, default: str) -> str:
        ...
        if cfg is None:
            return default
        try:
            return cfg.parser.get("arduino", key, fallback=default)
        except Exception:
            return default
    ...
    def _write_cfg(self, key: str, value: str) -> None:
        ...
        if cfg is None:
            return
        if not cfg.parser.has_section("arduino"):
            cfg.parser.add_section("arduino")
